...
public int overallScore()
{
   ...
   if (score1 < score2 && score1 < score3) {
      ...
      s1 = score1;
      s2 = score3; //FIXED: if score2 is the smallest use scores 1 and 3
   } else if (score3 < score1 && score3 < score2) {
      s1 = score1;
      s2 = score2;
   } else {
      s1 = score1; //FIXED: if all scores are equal set s1 to score1
      s2 = score2; //FIXED: if all scores are equal set s2 to score2
   }
   s = s1 + s2;
   return s;
}
...
{
   int s1, s2, s3;
   //FIXED: removed line checking if args is null
   if (args.length != 3) {
